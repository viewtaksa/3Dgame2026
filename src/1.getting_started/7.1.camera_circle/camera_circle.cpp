...
int main()
{
    ...
    // render loop
    // -----------
   // หาบรรทัดที่เป็น render loop แล้ววางทับส่วนการวาดกล่องเดิม
    while (!glfwWindowShouldClose(window))
    {
        processInput(window);

        glClearColor(0.1f, 0.1f, 0.1f, 1.0f); // ปรับพื้นหลังให้มืดเพื่อให้เห็นงานชัดขึ้น
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

        ourShader.use();
        float timeValue = glfwGetTime();
        ourShader.setFloat("time", timeValue); // ส่งเวลาไปให้ Shader

        // Camera Transformation (โค้ดเดิมของคุณที่ทำให้กล้องหมุน)
        ...
        // --- เริ่มสร้าง Kinetic Sculpture ---
        int grid = 12; // สร้างกล่อง 12x12 ชิ้น
        for (int i = 0; i < grid; i++)
        {
            for (int j = 0; j < grid; j++)
            {
                ...
                // 1. คำนวณตำแหน่งพื้นฐาน (Grid layout)
                float x = (i - grid / 2.0f) * 1.5f;
                float z = (j - grid / 2.0f) * 1.5f;

                // 2. ใช้ 3D Transformation Matrix คำนวณ Animation (หัวใจของงาน)
                // ระยะห่างจากศูนย์กลางเพื่อให้เกิดคลื่นวงกลม
                float dist = sqrt(x * x + z * z);
                float y = sin(timeValue * 2.0f - dist * 0.8f) * 2.0f;

                model = glm::translate(model, glm::vec3(x, y, z));

                // หมุนแต่ละชิ้นตามเวลาและตำแหน่ง (Kinetic Rotation)
                model = glm::rotate(model, timeValue + dist, glm::vec3(0.0f, 1.0f, 0.5f));

                // ย่อขยายตามจังหวะคลื่น
